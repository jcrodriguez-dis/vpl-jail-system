/**
 * version:		$Id: vpl-jail-server.cpp,v 1.6 2014-02-26 16:51:12 juanca Exp $
...
class Daemon{
	...
	void launch(int listen, bool sec){
		...
		int newSocket = ::accept(listen,(struct sockaddr *)&client,&slen);
		if(newSocket < 0)
			throw "Error accept";
		string IP;
		...
		if(pid==0){ //new process
			...
			try{
				close(listenSocket);
				if(sec)
					socket= new SSLSocket(newSocket);
				else
					socket= new Socket(newSocket);
				Jail jail(IP);
				jail.process(socket);
			}
